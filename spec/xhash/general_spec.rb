...
describe Xhash::General do
  describe '.store_data' do
    ...
    it 'fails to creates a customer by missing data' do
      ...
      begin
        ...
        expect(exception.response[:gender]).to eq(['The gender field is required.'])
  ...
  describe '.get_customer' do
    it 'successfully fetch customer data' do
      stub_request(:get, 'https://xhash.dev/api/get-customer/my-customer-id-1').to_return(
        body:
          JSON.dump(
            {
              payload: {
    ...
    it 'fails to fetch a customer data with bad id' do
      stub_request(:get, 'https://xhash.dev/api/get-customer/my-customer-id-2').to_return(
        body:
          JSON.dump(
            {
              customer_id: [
      ...
      begin
        ...
        expect(exception.response[:customer_id]).to eq(['The selected customer id is invalid.'])
      end
    end
  end
